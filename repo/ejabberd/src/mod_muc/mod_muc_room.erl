...
add_new_user(From, Nick, Packet, StateData) ->
    ...
    ?INFO_MSG("MUC limit =============== MaxUsers:~p,~n MaxAdminUsers:~p,~n NUsers:~p,~n Affiliation:~p,~n ServiceAffiliation:~p,~n NConferences:~p,~n MaxConferences:~p,~n Collision:~p~n",
		      [MaxUsers, MaxAdminUsers, NUsers, Affiliation, ServiceAffiliation, NConferences, MaxConferences, Collision]),
...
is_allowed_room_name_desc_limits(XEl, StateData) ->
	...
	case lists:keysearch("muc#roomconfig_roomname", 1,
			     jlib:parse_xdata_submit(XEl)) of
	    {value, {_, [N]}} ->
		length(N) =< gen_mod:get_module_opt(global,
	...
	case lists:keysearch("muc#roomconfig_roomdesc", 1,
			     jlib:parse_xdata_submit(XEl)) of
	    {value, {_, [D]}} ->
		length(D) =< gen_mod:get_module_opt(global,
