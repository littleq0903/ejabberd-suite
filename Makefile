...
build/ejabberd-websockets:
	cd repo/ejabberd-websockets && ./build.sh

build/mochi-mochiweb:
	cd repo/mochi-mochiweb && make && sudo cp ebin/* /lib/ejabberd/ebin

build/ejabberd_auth_mongodb_djangouser:
	cd repo/ejabberd_auth_mongodb_djangouser && ./build.sh && sudo cp ebin/* /lib/ejabberd/ebin

# rebuild commands

rebuild/exmpp:
	cd repo/exmpp && make && sudo make install

rebuild/erlmongo:
	cd repo/erlmongo && erl -make && sudo cp ebin/* /lib/ejabberd/ebin

rebuild/mod_chat_log_mongodb:
	cd repo/mod_chat_log_mongodb && ./build.sh && sudo cp ebin/* /lib/ejabberd/ebin

rebuild/mod_admin_extra_custom:
	cd repo/mod_admin_extra_custom && ./build.sh && sudo cp ebin/* /lib/ejabberd/ebin

rebuild/ejabberd:
	cd repo/ejabberd/src && sudo make install

rebuild/ejabberd-websockets:
	cd repo/ejabberd-websockets && ./build.sh

rebuild/ejabberd_auth_mongodb_djangouser:
	cd repo/ejabberd_auth_mongodb_djangouser && ./build.sh

rebuild: rebuild/mod_chat_log_mongodb rebuild/mod_admin_extra_custom rebuild/ejabberd rebuild/ejabberd-websockets build/mochi-mochiweb build/ejabberd_auth_mongodb_djangouser rebuild/exmpp rebuild/erlmongo
...
task2: build/mod_admin_extra_custom build/mod_chat_log_mongodb build/erlmongo build/ejabberd-websockets build/mochi-mochiweb build/ejabberd_auth_mongodb_djangouser
